...
export class Miners extends Kalan {

    direct_tax_points: number = 100
    taxed_goods: number = 0
    corvee_labor: number = 0

    initial_population:number = 2
    productivity_rate_modifier: number = 3
    unmodified_productivity: number = 132
    unmodified_development_growth: number = 16
    unmodified_population_growth: number = 37
    ...
    goods_produced_breakdown: string[] = ['Common Ores','Rare Ores','Gems']
    ...
    update_loyalty(goods: Map<string, Good>,mazar: Archivists) {
        ...
        if(mazar.bureaucratic_component.focus === BureaucraticFocus.Felak){
    ...
    update_efficiency(goods: Map<string, Good>,uzbaden: Rulers) {
        let beaurcracy_bonus = 0
        if(uzbaden.bureaucratic_component.focus === BureaucraticFocus.Felak){
    ...
    update_development_growth(baraz: Merchants): void {
        ...
        if (baraz.bureaucratic_component.focus === BureaucraticFocus.Felak) {
    ...
    update_population_growth(zigil: Clerics) {
        ...
        if (zigil.bureaucratic_component.focus === BureaucraticFocus.Felak) {
            this.development_growth *= zigil.bureaucratic_component.bonus
        }
    }

    update_unutilized_taxed_productivity() {
        this.unutilized_taxed_productivity = 100 - this.direct_tax_points - this.taxed_goods - this.corvee_labor
    }
