...
export class Foresters extends Kalan {
    ...
    goods_produced_breakdown: string[] = ['Food and Water','Artisanal Goods','Timber','Enchanted Charcoal']
    ...
    update_loyalty(goods: Map<string, Good>,mazar: Archivists) {
        ...
        if(mazar.bureaucratic_component.focus === BureaucraticFocus.Hund){
    ...
    update_efficiency(goods: Map<string, Good>,uzbaden: Rulers) {
        let beaurcracy_bonus = 0
        if(uzbaden.bureaucratic_component.focus === BureaucraticFocus.Hund){
    ...
    update_development_growth(baraz: Merchants): void {
        ...
        if (baraz.bureaucratic_component.focus === BureaucraticFocus.Hund) {
    ...
    update_population_growth(zigil: Clerics) {
        ...
        if (zigil.bureaucratic_component.focus === BureaucraticFocus.Hund) {
            this.development_growth *= zigil.bureaucratic_component.bonus
        }
    }

    update_unutilized_taxed_productivity() {
        this.unutilized_taxed_productivity = 100 - this.direct_tax_points - this.corvee_labor - this.taxed_goods
    }
